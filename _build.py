...
PATHS_REPLACE_VERSION_NUM = [
  "data/rc_forever_projectiles/dialog/about.json",
  "pack.mcmeta"
]
PATHS_REPLACE_LOAD_STATUS = [
  "data/rc_forever_projectiles/function/init.mcfunction"
]
...
for file_path in PATHS_REPLACE_LOAD_STATUS:
  find_replace_in_file(file_path, r'rc_forever_projectiles load\.status [0-9]*', f"rc_forever_projectiles load.status {major}")
  find_replace_in_file(file_path, r'rc_forever_projectiles_minor load\.status [0-9]*', f"rc_forever_projectiles_minor load.status {minor}")
